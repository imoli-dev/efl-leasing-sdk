...
`toRequestPayload()` produces an array with all fields, including nested structures for `address`, `countryOfOrigin` (as a Descriptor object with `id`, `name`, `version`; the SDK maps `countryOfOriginId` to a full Descriptor), `identityDocuments` (each converted via `toRequestPayload()`) and optional `statements`.
